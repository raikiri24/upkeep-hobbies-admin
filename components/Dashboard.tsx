...
const StatCard: React.FC<{ title: string; value: string; icon: React.ReactNode; trend: string; isPositive: boolean }> = ({ title, value, icon, trend, isPositive }) => (
  <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl relative overflow-hidden group hover:border-indigo-500/50 transition-colors">
    ...
    <div className="flex items-end gap-3 mt-1">
      ...
      <span className={`text-xs font-semibold mb-1 ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}>
        {trend}
      </span>
    </div>
  </div>
...
const Dashboard: React.FC = () => {
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-display font-bold text-white tracking-wide mb-6">Performance Overview</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-lg">
          <h3 className="text-lg font-bold text-white mb-6 font-display">Weekly Revenue Analysis</h3>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-lg">
          <h3 className="text-lg font-bold text-white mb-6 font-display">Sales by Category</h3>
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
