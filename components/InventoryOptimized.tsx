...
const InventoryOptimized: React.FC = () => {
  ...
  const categories = useMemo(() => {
    ...
    const uniqueCategories = Array.from(new Set(paginatedItems.data.map(item => item.category)));
    ...
    >
      ...
      <div className="sm:hidden p-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          <div className="flex justify-between items-center bg-slate-800/50 rounded p-2">
            ...
            <span className={`font-mono font-medium ${item.stock === 0 ? 'text-red-400' : item.stock <= 5 ? 'text-amber-400' : 'text-slate-300'}`}>
              {item.stock}
            </span>
          </div>
          <div className="flex justify-between items-center bg-slate-800/50 rounded p-2">
      ...
      <div className="hidden sm:flex items-center gap-4 p-4">
        ...
        <div className="text-sm font-mono text-center w-16">
          <span className={item.stock === 0 ? 'text-red-400' : item.stock <= 5 ? 'text-amber-400' : 'text-slate-300'}>
            {item.stock}
          </span>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-display font-bold text-white tracking-wide">Inventory Management</h2>
          <p className="text-slate-400 text-sm">
            {paginatedItems?.total || 0} total items • Page {currentPage} of {paginatedItems?.totalPages || 0}
          </p>
        </div>
        <button 
      ...
      {paginatedItems && paginatedItems.totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-sm text-slate-400 text-center">
            Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, paginatedItems.total)} of {paginatedItems.total} items
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => setCurrentPage(prev => Math.min(paginatedItems.totalPages, prev + 1))}
