...
import { Gamepad2, Lock, User as UserIcon, Loader2, Mail, AlertCircle } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const errorMessage = err.message || err.error_description || 'An error occurred during Google authentication';
    ...
    try {
      if (isRegisterMode) {
        ...
        if (response.success && response.user && response.token) {
          ...
          setError(response.message || 'Invalid credentials. Try admin@upkeep.com / password123');
        }
      }
    } catch (err) {
          ...
          {!googleConfigured && (
            <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-yellow-400 text-sm font-medium">Google OAuth Not Configured</p>
                  <p className="text-yellow-300/70 text-xs mt-1">
                    Please set VITE_GOOGLE_CLIENT_ID in your environment variables to enable Google sign-in.
                  </p>
                </div>
              </div>
            </div>
