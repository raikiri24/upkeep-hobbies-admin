  ...
  return useQuery({
    ...
    queryFn: async (): Promise<PaginatedResponse<Sale>> => {
      ...
      if (filters?.startDate) {
        filteredSales = filteredSales.filter(
          (sale) => sale.timestamp >= filters.startDate!
        );
      }
      if (filters?.endDate) {
        filteredSales = filteredSales.filter(
          (sale) => sale.timestamp <= filters.endDate!
        );
      }
      ...
      if (filters?.staffId) {
        filteredSales = filteredSales.filter(
          (sale) => sale.staffId === filters.staffId
        );
      }
...
export const useCreateCustomer = () => {
  ...
  return useMutation({
    mutationFn: (
      customerData: Parameters<typeof apiService.createCustomer>[0]
